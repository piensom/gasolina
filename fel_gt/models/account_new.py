...
class AccountMove(models.Model):
    ...
    fecha_fel = fields.Char(string='Fecha DTE.', readonly=True, states={'draft': [('readonly', False)]}, copy=False)
    frases_fel = fields.Many2many('dte.frases', string="Frases")
    ...
    def dte_documento(self):
        ...
        if factura.move_type in ['out_invoice', 'out_refund', 'in_invoice'] and not factura.firma_fel and factura.amount_total != 0:
        #if factura.move_type in ['out_invoice', 'in_invoice'] and not factura.firma_fel and factura.amount_total != 0:
            ...
            if self.invoice_date:
                ...
                fecha = factura.invoice_date.strftime('%Y-%m-%dT%H:%M:%S')
                fecha_str = fecha
            else:    
                fecha = fields.Datetime.context_timestamp(self.with_context(tz=self.env.user.tz), datetime.datetime.now() - timedelta(hours = 6))
                fecha_str = str(fecha.strftime(formato_fecha))
                
            self.fecha_fel = fecha_str
            incluir_numero_acceso = False    
                
            if ('numero_acceso' in factura.fields_get()):
                incluir_numero_acceso = True
                numero_acceso = factura.numero_acceso
                
            if incluir_numero_acceso and factura.es_contingencia:
                DatosGenerales = etree.SubElement(DatosEmision, DTE_NS+"DatosGenerales", CodigoMoneda=moneda, FechaHoraEmision=fecha_str, NumeroAcceso=numero_acceso ,Tipo=tipo_documento_fel)
                
            else:    
                DatosGenerales = etree.SubElement(DatosEmision, DTE_NS+"DatosGenerales", CodigoMoneda=moneda, FechaHoraEmision=fecha_str, Tipo=tipo_documento_fel)
            ...
            Direccion.text = (factura.partner_id.street or 'Ciudad') + ' ' + (factura.partner_id.street2 or '') + ' ' + (factura.partner_id.city or '') + ' ' + (factura.partner_id.state_id.name or '')
            ...
            if tipo_documento_fel not in ['NDEB', 'NCRE', 'RECI', 'NABN', 'FESP']:
                ...
                for f in factura.journal_id.frases_fel:
                    frase = etree.SubElement(frases, DTE_NS+'Frase')
                    frase.set('CodigoEscenario', f.codigo_escenario)
                    frase.set('TipoFrase', f.tipo_frase)
                
                for x in factura.frases_fel:
                    frase = etree.SubElement(frases, DTE_NS+'Frase')
                    frase.set('CodigoEscenario', x.codigo_escenario)
                    frase.set('TipoFrase', x.tipo_frase)
            ...
            for linea in factura.invoice_line_ids:
                ...
                precio_unitario_base = round((linea.price_total/linea.quantity)/1.12,6)
                total_linea = round(precio_unitario * linea.quantity,6)
                total_linea_base = round(precio_unitario_base * linea.quantity,6)
                total_impuestos = round(total_linea - total_linea_base,6)
                ...
                ### Posible Solucion a problema redondeo ###
                ...
                Cantidad.text = str(round(linea.quantity,2))
                ...
                Descuento.text = '{:.2f}'.format(descuento)
            ...
            if tipo_documento_fel in ['NDEB', 'NCRE'] or tipo_documento_fel in ['FCAM'] or (tipo_documento_fel in ['FACT', 'FCAM'] and factura.tipo_gasto == 'importacion') or tipo_documento_fel in ['FESP']:
                ...
                if tipo_documento_fel in ['NDEB', 'NCRE']:
                    ...
                    elif factura.pos_order_ids:
                        ReferenciasNota = etree.SubElement(Complemento, CNO_NS+"ReferenciasNota", FechaEmisionDocumentoOrigen=str(factura.pos_order_ids.refunded_order_ids.account_move.invoice_date), MotivoAjuste="-", NumeroAutorizacionDocumentoOrigen=factura.pos_order_ids.refunded_order_ids.account_move.firma_fel, NumeroDocumentoOrigen=factura.pos_order_ids.refunded_order_ids.account_move.numero_fel, SerieDocumentoOrigen=factura.pos_order_ids.refunded_order_ids.account_move.serie_fel, Version="0.0", nsmap=NSMAP_REF)
                    
                    else:
                        #ReferenciasNota = etree.SubElement(Complemento, CNO_NS+"ReferenciasNota", RegimenAntiguo="Antiguo", FechaEmisionDocumentoOrigen=str(factura.reversed_entry_id.fecha_fel), MotivoAjuste="-", NumeroAutorizacionDocumentoOrigen=factura.reversed_entry_id.firma_fel, NumeroDocumentoOrigen=factura.reversed_entry_id.ref.split("-")[1], SerieDocumentoOrigen=factura.reversed_entry_id.ref.split("-")[0], Version="0.0", nsmap=NSMAP_REF)
    ...
    def dte_anulacion(self):
        ...
        if factura.firma_fel:
            ...
            if tipo_documento_fel == "FESP" and factura.partner_id.cui:
                nit_receptor = factura.partner_id.cui
            formato_fecha = "%Y-%m-%dT%H:%M:%S"
            fecha_documento_anular = self.fecha_fel
            fecha = fields.Datetime.context_timestamp(self.with_context(tz=self.env.user.tz), datetime.datetime.now()- timedelta(hours = 6))
            if fecha:
                fecha_str = str(fecha.strftime(formato_fecha))
